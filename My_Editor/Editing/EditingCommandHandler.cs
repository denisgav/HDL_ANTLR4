...
namespace My_Editor.Editing
{
	...
	static class EditingCommandHandler
	{
		...
		static EditingCommandHandler()
		{
            ...
            CommandBindings.Add(new CommandBinding(AvalonEditCommands.ShowCodeCompletitionWindow, OnShowCodeCompletitionWindow));
        ...
        #region ShowCodeCompletitionWindow
        static void OnShowCodeCompletitionWindow(object target, ExecutedRoutedEventArgs args)
        {
            TextArea textArea = GetTextArea(target);
            if (textArea.Lexter != null)
            {
                if (textArea != null && textArea.Document != null)
                {
                    My_Editor.CodeCompletion.CodeCompletionList complList;
                    complList = textArea.Lexter.CodeCompletionList;
                    complList.Show();
                }
            }
        }
		...
		static void OnIndentSelection(object target, ExecutedRoutedEventArgs args)
		{
			...
			if (textArea != null && textArea.Document != null) {
				using (textArea.Document.RunUpdate()) {
					...
					textArea.Lexter.IndentationStrategy.IndentLines(textArea.Document, start, end);
				}
				textArea.Caret.BringCaretToView();
				args.Handled = true;
			}
